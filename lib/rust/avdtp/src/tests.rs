...
const CMD_DISCOVER: &'static [u8] = &[
    0x40, // TxId (4), Single Packet (0b00), Command (0b00)
...
fn command_not_supported_response() {
    ...
    // TxId 4, DelayReport, which is not implemented
...
fn invalid_signal_id_responds_error() {
    ...
    // This is TxId 4, Signal Id 0b110000, which is invalid.
    ...
    // same TxId, and the same (invalid) signal identifier.
...
fn discover_invalid_length() {
    ...
    // TxId 4, Discover, which doesn't have a payload, but we're including one.
...
fn discover_command_works() {
    ...
    let txid_raw = received[0] & 0xF0;

    let response: &[u8] = &[
        txid_raw << 4 | 0x0 << 2 | 0x2, // TXID (same), Single (0b00), Response Accept (0b10)
        0x01,                           // Discover
    ...
    let endpoints = match complete {
        Poll::Ready(Ok(response)) => response.endpoints,
...
fn discover_reject_command() {
    ...
    let response: &[u8] = &[
        txid_raw << 4 | 0x0 << 2 | 0x3, // TXID (same), Single (0b00), Response Reject (0b11)
        0x01,                           // Discover
        0x31,                           // BAD_STATE
    ];
