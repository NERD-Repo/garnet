...
func (ns *Netstack) addEth(interfaceConfig netstack.InterfaceConfig, device ethernet.DeviceInterface) error {
	var client *eth.Client
	return ns.addEndpoint(func(ifs *ifState) (stack.LinkEndpoint, error) {
		var err error
		client, err = eth.NewClient("netstack", interfaceConfig.Name, &device, ns.arena, ifs.stateChange)
		if err != nil {
			return nil, err
		}
		ifs.eth = client
