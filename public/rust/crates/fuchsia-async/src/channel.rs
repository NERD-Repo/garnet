...
impl Channel {
    ...
    pub fn recv_from(
        &self, buf: &mut MessageBuf, cx: &mut task::Context,
    ...
    pub async fn chain_server<F, Fut>(
        self, mut callback: F,
    ) -> Result<(Channel, MessageBuf), zx::Status>
    where
...
mod tests {
    ...
    use fuchsia_zircon::{self as zx, prelude::*, MessageBuf};
    use futures::prelude::*;
    ...
    fn can_receive() {
        ...
        let receiver = receiver.on_timeout(1000.millis().after_now(), || panic!("timeout"));
    ...
    fn chain_server() {
        ...
        let mut count = 0;
        let receiver = f_rx
            .chain_server(async move |chan, buf| {
