...
use futures::{pin_mut, task, Future, FutureExt, Poll};
    ...
    F: Future<Output = ()> + 'static,
{
    Inner::spawn_local(
        &EHandle::local().inner,
        LocalFutureObj::from(Box::new(future)),
    );
}
...
impl Executor {
        ...
        F::Output: Send + 'static,
    {
        self.inner.threadiness.require_multithreaded().expect(
            "Error: called `run` on executor after using `spawn_local`. Use `run_singlethreaded` \
             instead.",
        );
        ...
        Inner::spawn(
            &self.inner,
            Box::new(future.map(move |fut_result| {
                let (lock, cvar) = &*pair2;
    ...
    fn worker_lifecycle(inner: Arc<Inner>, timers: Option<TimerHeap>) {
        let mut executor: EHandle = EHandle {
            inner: inner.clone(),
        };
        executor
            .clone()
            .set_local(timers.unwrap_or(TimerHeap::new()));
...
impl Inner {
    ...
    fn spawn_local(arc_self: &Arc<Self>, future: LocalFutureObj<'static, ()>) {
        arc_self.threadiness.require_singlethreaded().expect(
            "Error: called `spawn_local` after calling `run` on executor. Use `spawn` or \
             `run_singlethreaded` instead.",
        );
        Inner::spawn(
            arc_self,
