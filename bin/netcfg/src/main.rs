...
#![deny(warnings)]
#![feature(async_await, await_macro, nll)]
...
fn main() -> Result<(), failure::Error> {
    ...
    let mut interface_config = interface::FileBackedConfig::load(&"/data/net.cfg.json")?;
    ...
    let mut device_name_stream = futures::stream::iter(default_device_name).chain(
        netstack.take_event_stream().take(1).try_filter_map(
                    ...
                    if is_physical(device_info.features) {
                            ...
                            })?.into_zx_channel();
                        {
                            let interface_config =
                                interface_config.get(interface::Identifier::new(
                                    topological_path,
                                    interface::MacAddress::new(device_info.mac),
                                    device_info.features & INFO_FEATURE_WLAN == INFO_FEATURE_WLAN,
                                ))?;

                            let () = netstack
                                .add_ethernet_device(
                                    interface_config.into_mut(),
