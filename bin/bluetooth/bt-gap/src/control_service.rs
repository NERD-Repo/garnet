...
pub fn make_control_service(
    ...
    unsafe_many_futures!(Output, [A, B, C  D, E, F, G, H, I, J, L, M, N]);
        ...
        .try_for_each(move |evt| match evt {
            ...
            ControlRequest::SetDiscoverable {
                ...
                let fut = if discoverable {
                    ...
                    Left(HostDispatcher::set_discoverable(state.read().host.clone()).map_ok(
                            move |(mut resp, token)| {
            ...
            ControlRequest::GetKnownRemoteDevices { .. } => Output::N(future::ready(Ok(()))),
