...
use fidl::endpoints2;
use fidl_fuchsia_wlan_device_service::{self as wlan_service, DeviceServiceMarker, DeviceServiceProxy};
use fidl_fuchsia_wlan_sme::{self as fidl_sme, ConnectResultCode, ConnectTransactionEvent, ScanTransactionEvent};
...
async fn do_iface(cmd: opts::IfaceCmd, wlan_svc: WlanSvc) -> Result<(), Error> {
    match cmd {
        ...
        opts::IfaceCmd::Delete { phy_id, iface_id } => {
            ...
            let status = await!(wlan_svc.destroy_iface(&mut req)).context("error destroying iface")?;
...
async fn do_client(cmd: opts::ClientCmd, wlan_svc: WlanSvc) -> Result<(), Error> {
    match cmd {
        ...
        opts::ClientCmd::Status { iface_id } => {
            let sme = await!(get_client_sme(wlan_svc, iface_id))?;
            let status = await!(sme.status()).context("error getting status")?;
            ...
            if !status.connecting_to_ssid.is_empty() {
                println!("Connecting to '{}'", String::from_utf8_lossy(&status.connecting_to_ssid));
            }
            Ok(())
        }
    }
...
async fn handle_scan_transaction(scan_txn: fidl_sme::ScanTransactionProxy) -> Result<(), Error> {
    ...
    while let Some(e) = await!(events.next()) {
        match e? {
            ...
            ScanTransactionEvent::OnFinished { } => done = true,
