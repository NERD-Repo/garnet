...
where
    F: Fn(LogMessage, usize) + Send + Sync + 'static,
{
...
async fn run_listener<F>(handle: zx::Handle, callback: F) -> Result<(), zx::Status>
where
    F: Fn(LogMessage, usize) + Send + Sync + 'static,
{
    'outer: loop {
        // TODO: change OnSignals to wrap this so that is is not created again and again.
        await!(async::OnSignals::new(&handle, Signals::LOG_READABLE))?;
        'inner: loop {
