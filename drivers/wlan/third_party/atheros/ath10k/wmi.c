...
int ath10k_wmi_cmd_send(struct ath10k* ar, struct sk_buff* skb, uint32_t cmd_id) {
    ...
    if (cmd_id == WMI_CMD_UNSUPPORTED) {
        ath10k_info("wmi command %d is not supported by firmware\n",
                    cmd_id);
        return ret;
    }
...
static void ath10k_wmi_event_scan_started(struct ath10k* ar) {
    ...
    case ATH10K_SCAN_ABORTING:
        ath10k_info("received scan started event in an invalid scan state: %s (%d)\n",
...
static void ath10k_wmi_event_scan_start_failed(struct ath10k* ar) {
    ...
    case ATH10K_SCAN_RUNNING:
    case ATH10K_SCAN_ABORTING:
        ath10k_info("received scan start failed event in an invalid scan state: %s (%d)\n",
                    ath10k_scan_state_str(ar->scan.state),
                    ar->scan.state);
        break;
    case ATH10K_SCAN_STARTING:
...
static void ath10k_wmi_event_scan_completed(struct ath10k* ar) {
    ...
    case ATH10K_SCAN_STARTING:
         ...
         */
        ath10k_info("received scan completed event in an invalid scan state: %s (%d)\n",
                    ath10k_scan_state_str(ar->scan.state),
                    ar->scan.state);
        break;
    case ATH10K_SCAN_RUNNING:
...
static void ath10k_wmi_event_scan_bss_chan(struct ath10k* ar) {
    ...
    case ATH10K_SCAN_IDLE:
    case ATH10K_SCAN_STARTING:
        ath10k_info("received scan bss chan event in an invalid scan state: %s (%d)\n",
                    ath10k_scan_state_str(ar->scan.state),
                    ar->scan.state);
        break;
    case ATH10K_SCAN_RUNNING:
...
static void ath10k_wmi_event_scan_foreign_chan(struct ath10k* ar, uint32_t freq) {
    ...
    case ATH10K_SCAN_IDLE:
    case ATH10K_SCAN_STARTING:
        ath10k_info("received scan foreign chan event in an invalid scan state: %s (%d)\n",
                    ath10k_scan_state_str(ar->scan.state),
                    ar->scan.state);
        break;
    case ATH10K_SCAN_RUNNING:
...
int ath10k_wmi_event_scan(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to parse scan event: %d\n", ret);
        return ret;
    }
    ...
    case WMI_SCAN_EVENT_FOREIGN_CHANNEL:
        ...
        break;
    case WMI_SCAN_EVENT_START_FAILED:
        ath10k_info("received scan start failure event\n");
...
int ath10k_wmi_event_mgmt_rx(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to parse mgmt rx event: %d\n", ret);
        dev_kfree_skb(skb);
        return ret;
    }
...
void ath10k_wmi_event_chan_info(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to parse chan info event: %d\n", ret);
        return;
    }
    ...
    case ATH10K_SCAN_IDLE:
    case ATH10K_SCAN_STARTING:
        ath10k_info("received chan info event without a scan request, ignoring\n");
        goto exit;
    case ATH10K_SCAN_RUNNING:
    case ATH10K_SCAN_ABORTING:
        break;
    ...
    if (idx >= ARRAY_SIZE(ar->survey)) {
        ath10k_info("chan info: invalid frequency %d (idx %d out of bounds)\n",
                    freq, idx);
        goto exit;
    }
...
void ath10k_wmi_event_echo(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to parse echo: %d\n", ret);
        return;
    }
...
void ath10k_wmi_event_vdev_start_resp(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to parse vdev start event: %d\n", ret);
        return;
    }

    if (WARN_ON(arg.status)) {
        return;
    }
...
void ath10k_wmi_event_peer_sta_kickout(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to parse peer kickout event: %d\n",
                    ret);
        return;
    }
    ...
    if (!sta) {
        ath10k_info("Spurious quick kickout for STA %pM\n",
                    arg.mac_addr);
        goto exit;
    }
...
static void ath10k_wmi_update_tim(struct ath10k* ar,
    ...
    if (tim_info->tim_changed) {
        ...
        if (sizeof(arvif->u.ap.tim_bitmap) < tim_len) {
            ath10k_info("SWBA TIM field is too big (%u), truncated it to %zu",
    ...
    if (pvm_len < arvif->u.ap.tim_len) {
        ...
        if (skb_put(bcn, expand_size)) {
            ...
            ath10k_info("tim expansion failed\n");
        }
    }

    if (pvm_len > tim_len) {
        ath10k_info("tim pvm length is too great (%d)\n", pvm_len);
        return;
    }
...
static int ath10k_wmi_op_pull_swba_ev(struct ath10k* ar, struct sk_buff* skb,
    ...
    for (i = 0, map = ev->vdev_map; map; map >>= 1) {
        ...
        if (ev->bcn_info[i].tim_info.tim_len >
            ...
            ath10k_info("refusing to parse invalid swba structure\n");
            return -EPROTO;
        }
...
void ath10k_wmi_event_host_swba(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to parse swba event: %d\n", ret);
        return;
    }
    ...
    for (; map; map >>= 1, vdev_id++) {
        ...
        if (i >= WMI_MAX_AP_VDEV) {
            ath10k_info("swba has corrupted vdev map\n");
            break;
        }
        ...
        if (arvif == NULL) {
            ath10k_info("no vif for vdev_id %d found\n",
                        vdev_id);
            continue;
        }
        ...
        if (!bcn) {
            ath10k_info("could not get mac80211 beacon\n");
            continue;
        }
        ...
        if (arvif->beacon) {
            ...
            case ATH10K_BEACON_SENT:
                break;
            case ATH10K_BEACON_SCHEDULED:
                ath10k_info("SWBA overrun on vdev %d, skipped old beacon\n",
                            arvif->vdev_id);
                break;
            case ATH10K_BEACON_SENDING:
                ath10k_info("SWBA overrun on vdev %d, skipped new beacon\n",
                            arvif->vdev_id);
                dev_kfree_skb(bcn);
                goto skip;
            }
        ...
        if (!arvif->beacon_buf) {
            ...
            if (ret) {
                ath10k_info("failed to map beacon: %d\n",
                            ret);
            ...
            if (bcn->len > IEEE80211_MAX_FRAME_LEN) {
                ath10k_info("trimming beacon %d -> %d bytes!\n",
...
static void ath10k_dfs_radar_report(struct ath10k* ar,
    ...
    if (!ch) {
        ath10k_info("failed to derive channel for radar pulse, treating as radar\n");
        goto radar_detected;
    }
...
void ath10k_wmi_event_dfs(struct ath10k* ar,
    ...
    while (i < buf_len) {
        if (i + sizeof(*tlv) > buf_len) {
            ath10k_info("too short buf for tlv header (%d)\n",
                        i);
            return;
        }
        ...
        case PHYERR_TLV_TAG_RADAR_PULSE_SUMMARY:
            if (i + sizeof(*tlv) + sizeof(*rr) > buf_len) {
                ath10k_info("too short radar pulse summary (%d)\n",
                            i);
                return;
            }
        ...
        case PHYERR_TLV_TAG_SEARCH_FFT_REPORT:
            if (i + sizeof(*tlv) + sizeof(*fftr) > buf_len) {
                ath10k_info("too short fft report (%d)\n",
                            i);
                return;
            }
...
void ath10k_wmi_event_spectral_scan(struct ath10k* ar,
    ...
    while (i < buf_len) {
        if (i + sizeof(*tlv) > buf_len) {
            ath10k_info("failed to parse phyerr tlv header at byte %d\n",
                        i);
            return;
        }
        ...
        if (i + sizeof(*tlv) + tlv_len > buf_len) {
            ath10k_info("failed to parse phyerr tlv payload at byte %d\n",
                        i);
            return;
        }
        ...
        case PHYERR_TLV_TAG_SEARCH_FFT_REPORT:
            if (sizeof(*fftr) > tlv_len) {
                ath10k_info("failed to parse fft report at byte %d\n",
                            i);
                return;
            }
...
int ath10k_wmi_op_pull_phyerr_ev(struct ath10k* ar,
    ...
    if (left_len < sizeof(*phyerr)) {
        ath10k_info("wrong phyerr event head len %d (need: >=%zd)\n",
...
void ath10k_wmi_event_phyerr(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to parse phyerr event hdr: %d\n", ret);
        return;
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        if (ret) {
            ath10k_info("failed to parse phyerr event (%d)\n",
                        i);
            return;
        }
        ...
        if (left_len < buf_len) {
            ath10k_info("single event (%d) wrong buf len\n", i);
            return;
        }
...
void ath10k_wmi_event_roam(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to parse roam event: %d\n", ret);
        return;
    }
    ...
    if (reason >= WMI_ROAM_REASON_MAX)
        ath10k_info("ignoring unknown roam event reason %d on vdev %i\n",
                    reason, vdev_id);
    ...
    case WMI_ROAM_REASON_HO_FAILED:
        ath10k_info("ignoring not implemented roam event reason %d on vdev %i\n",
                    reason, vdev_id);
        break;
    }
}
...
void ath10k_wmi_event_debug_print(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (i == sizeof(buf) - 1) {
        ath10k_info("wmi debug print truncated: %d\n", skb->len);
    }
...
void ath10k_wmi_event_wow_wakeup_host(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to parse wow wakeup event: %d\n", ret);
        return;
    }
...
static uint8_t ath10k_tpc_config_get_rate(struct ath10k* ar,
    ...
    case WMI_TPC_TABLE_TYPE_CDD:
        ...
        break;
    default:
        ath10k_info("unknown wmi tpc table type: %d\n", type);
        tpc = 0;
        break;
    }
...
ath10k_wmi_main_op_pull_svc_rdy_ev(struct ath10k* ar, struct sk_buff* skb,
    ...
    if (skb->len <
            arg->num_mem_reqs * sizeof(arg->mem_reqs[0])) {
        return -EPROTO;
    }
...
static void ath10k_wmi_event_service_ready_work(struct work_struct* work) {
    ...
    if (!skb) {
        ath10k_info("invalid service ready event skb\n");
        return;
    }

    ret = ath10k_wmi_pull_svc_rdy(ar, skb, &arg);
    if (ret) {
        ath10k_info("failed to parse service ready: %d\n", ret);
        return;
    }
    ...
    ar->fw_version_major =
        (arg.sw_ver0 & 0xff000000) >> 24;
    ar->fw_version_minor = (arg.sw_ver0 & 0x00ffffff);
    ar->fw_version_release =
        (arg.sw_ver1 & 0xffff0000) >> 16;
    ...
    if (ar->num_rf_chains > ar->max_spatial_stream) {
        ath10k_info("hardware advertises support for more spatial streams than it should (%d > %d)\n",
    ...
    if (num_mem_reqs > WMI_MAX_MEM_REQS) {
        ath10k_info("requested memory chunks number (%d) exceeds the limit\n",
                    num_mem_reqs);
        return;
    }
...
int ath10k_wmi_event_ready(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to parse ready event: %d\n", ret);
        return ret;
    }
...
static int ath10k_wmi_event_pdev_bss_chan_info(struct ath10k* ar,
    ...
    idx = freq_to_idx(ar, freq);
    if (idx >= ARRAY_SIZE(ar->survey)) {
        ath10k_info("bss chan info: invalid frequency %d (idx %d out of bounds)\n",
                    freq, idx);
        goto exit;
    }
...
static void ath10k_wmi_process_rx(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ret) {
        ath10k_info("failed to process wmi rx: %d\n", ret);
    }
}
...
int ath10k_wmi_connect(struct ath10k* ar) {
    ...
    if (status) {
        ath10k_info("failed to connect to WMI CONTROL service status: %d\n",
                    status);
        return status;
    }
...
ath10k_wmi_op_gen_pdev_set_param(struct ath10k* ar, uint32_t id, uint32_t value) {
    ...
    if (id == WMI_PDEV_PARAM_UNSUPPORTED) {
        ath10k_info("pdev param %d not supported by firmware\n",
                    id);
...
void ath10k_wmi_main_op_fw_stats_fill(struct ath10k* ar,
    ...
    if (!pdev) {
        ath10k_info("failed to get pdev stats\n");
        goto unlock;
    }
...
static struct sk_buff*
ath10k_wmi_op_gen_echo(struct ath10k* ar, uint32_t value) {
...
ath10k_wmi_barrier(struct ath10k* ar) {
    ...
    if (ret) {
        ath10k_info("failed to submit wmi echo: %d\n", ret);
        return ret;
    }
