...
static int ath10k_spectral_scan_config(struct ath10k* ar,
    ...
    if (res < 0) {
        ath10k_info("failed to enable spectral scan: %d\n", res);
        return res;
    }
    ...
    if (res < 0) {
        ath10k_info("failed to configure spectral scan: %d\n", res);
        return res;
    }
    ...
    if (strncmp("trigger", buf, 7) == 0) {
        if (ar->spectral.mode == SPECTRAL_MANUAL ||
            ...
            res = ath10k_spectral_scan_config(ar,
                                              ar->spectral.mode);
            if (res < 0) {
                ath10k_info("failed to reconfigure spectral scan: %d\n",
                            res);
            }
            res = ath10k_spectral_scan_trigger(ar);
            if (res < 0) {
                ath10k_info("failed to trigger spectral scan: %d\n",
                            res);
            }
        } else {
            res = -EINVAL;
