...
static inline struct sk_buff* ath10k_htt_rx_netbuf_pop(struct ath10k_htt* htt) {
    ...
    if (htt->rx_ring.fill_cnt == 0) {
        ath10k_info("tried to pop sk_buff from an empty rx ring\n");
        return NULL;
    }
    ...
    while (msdu_count--) {
        ...
        if (!is_offload) {
            ...
            if (!(rxd->attention.flags &
                    RX_ATTENTION_FLAGS_MSDU_DONE)) {
                ath10k_info("tried to pop an incomplete frame, oops!\n");
                return -EIO;
            }
        }
...
int ath10k_htt_rx_alloc(struct ath10k_htt* htt) {
    ...
    if (!is_power_of_2(htt->rx_ring.size)) {
        ath10k_info("htt rx ring size is not power of 2\n");
        return -EINVAL;
    }
...
static int ath10k_htt_rx_crypto_param_len(struct ath10k* ar,
    ...
    ath10k_info("unsupported encryption type %d\n", type);
    return 0;
}
...
static void ath10k_htt_rx_h_rates(struct ath10k* ar,
    ...
    case HTT_RX_VHT_WITH_TXBF:
        ...
        if (mcs > 0x09) {
            ath10k_info("invalid MCS received %u\n", mcs);
            ath10k_info("rxd %08x mpdu start %08x %08x msdu start %08x %08x ppdu start %08x %08x %08x %08x %08x\n",
                        rxd->attention.flags,
            ...
            ath10k_info("msdu end %08x mpdu end %08x\n",
...
static bool ath10k_htt_rx_amsdu_allowed(struct ath10k* ar,
    ...
    if (!rx_status->freq) {
        ath10k_info("no channel configured; ignoring frame(s)!\n");
        return false;
    }
...
static int ath10k_htt_rx_handle_amsdu(struct ath10k_htt* htt) {
    ...
    if (ret < 0) {
        ath10k_info("rx ring became corrupted: %d\n", ret);
        __skb_queue_purge(&amsdu);
...
static void ath10k_htt_rx_tx_compl_ind(struct ath10k* ar,
    ...
    case HTT_DATA_TX_STATUS_DOWNLOAD_FAIL:
        ...
        break;
    default:
        ath10k_info("unhandled tx completion status %d\n", status);
        tx_done.status = HTT_TX_COMPL_STATE_DISCARD;
        break;
    }
    ...
    for (i = 0; i < resp->data_tx_completion.num_msdus; i++) {
        ...
        if (!kfifo_put(&htt->txdone_fifo, tx_done)) {
            ath10k_info("txdone fifo overrun, msdu_id %d status %d\n",
...
static void ath10k_htt_rx_addba(struct ath10k* ar, struct htt_resp* resp) {
    ...
    peer = ath10k_peer_find_by_id(ar, peer_id);
    if (!peer) {
        ath10k_info("received addba event for invalid peer_id: %hu\n",
                    peer_id);
        spin_unlock_bh(&ar->data_lock);
        return;
    }
    ...
    arvif = ath10k_get_arvif(ar, peer->vdev_id);
    if (!arvif) {
        ath10k_info("received addba event for invalid vdev_id: %u\n",
                    peer->vdev_id);
        spin_unlock_bh(&ar->data_lock);
        return;
    }
...
static int ath10k_htt_rx_h_rx_offload(struct ath10k* ar,
    ...
    while ((msdu = __skb_dequeue(list))) {
        ...
        if (skb_tailroom(msdu) < rx->msdu_len) {
            ath10k_info("dropping frame: offloaded rx msdu is too long!\n");
...
static int ath10k_htt_rx_in_ord_ind(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (skb->len < msdu_count * sizeof(*resp->rx_in_ord_ind.msdu_descs)) {
        ath10k_info("dropping invalid in order rx indication\n");
        return -EINVAL;
    }
    ...
    if (ret < 0) {
        ath10k_info("failed to pop paddr list: %d\n", ret);
        htt->rx_confused = true;
        return -EIO;
    }
    ...
    while (!skb_queue_empty(&list)) {
        ...
        default:
            /* Should not happen. */
            ath10k_info("failed to extract amsdu: %d\n", ret);
            htt->rx_confused = true;
            __skb_queue_purge(&list);
            return -EIO;
        }
...
static void ath10k_htt_rx_tx_fetch_ind(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (unlikely(skb->len < len)) {
        ath10k_info("received corrupted tx_fetch_ind event: buffer too short\n");
        return;
    }
    ...
    if (unlikely(skb->len < len)) {
        ath10k_info("received corrupted tx_fetch_ind event: too many records/resp_ids\n");
        return;
    }
    ...
    if (!ar->htt.tx_q_state.enabled) {
        ath10k_info("received unexpected tx_fetch_ind event: not enabled\n");
        return;
    }

    if (ar->htt.tx_q_state.mode == HTT_TX_MODE_SWITCH_PUSH) {
        ath10k_info("received unexpected tx_fetch_ind event: in push mode\n");
        return;
    }
    ...
    for (i = 0; i < num_records; i++) {
        ...
        if (unlikely(peer_id >= ar->htt.tx_q_state.num_peers) ||
            ...
            ath10k_info("received out of range peer_id %hu tid %hhu\n",
                        peer_id, tid);
            continue;
        }
        ...
        if (unlikely(!txq)) {
            ath10k_info("failed to lookup txq for peer_id %hu tid %hhu\n",
                        peer_id, tid);
            continue;
        }
    ...
    ret = ath10k_htt_tx_fetch_resp(ar,
                                   ...
                                   num_records);
    if (unlikely(ret)) {
        ath10k_info("failed to submit tx fetch resp for token 0x%08x: %d\n",
...
static void ath10k_htt_rx_tx_fetch_confirm(struct ath10k* ar,
    ...
    if (unlikely(skb->len < len)) {
        ath10k_info("received corrupted tx_fetch_confirm event: buffer too short\n");
        return;
    }
    ...
    if (unlikely(skb->len < len)) {
        ath10k_info("received corrupted tx_fetch_confirm event: resp_ids buffer overflow\n");
        return;
    }
...
static void ath10k_htt_rx_tx_mode_switch_ind(struct ath10k* ar,
    ...
    if (unlikely(skb->len < len)) {
        ath10k_info("received corrupted tx_mode_switch_ind event: buffer too short\n");
        return;
    }
    ...
    if (unlikely(skb->len < len)) {
        ath10k_info("received corrupted tx_mode_switch_mode_ind event: too many records\n");
        return;
    }
    ...
    case HTT_TX_MODE_SWITCH_PUSH_PULL:
        break;
    default:
        ath10k_info("received invalid tx_mode_switch_mode_ind mode %d, ignoring\n",
                    mode);
        return;
    }
...
ath10k_update_per_peer_tx_stats(struct ath10k* ar,
    ...
    if (txrate.flags == WMI_RATE_PREAMBLE_VHT && txrate.mcs > 9) {
        ath10k_info("Invalid VHT mcs %hhd peer stats",  txrate.mcs);
        return;
    }

    if (txrate.flags == WMI_RATE_PREAMBLE_HT &&
            (txrate.mcs > 7 || txrate.nss < 1)) {
        ath10k_info("Invalid HT mcs %hhd nss %hhd peer stats",
    ...
    if (txrate.flags == WMI_RATE_PREAMBLE_CCK ||
        ...
        if (!is_valid_legacy_rate(rate)) {
            ath10k_info("Invalid legacy rate %hhd peer stats",
                        rate);
            return;
        }
...
static void ath10k_htt_fetch_peer_stats(struct ath10k* ar,
    ...
    if (skb->len < sizeof(struct htt_resp_hdr) + num_ppdu * ppdu_len) {
        ath10k_info("Invalid peer stats buf length %d\n", skb->len);
        return;
    }
    ...
    peer = ath10k_peer_find_by_id(ar, peer_id);
    if (!peer) {
        ath10k_info("Invalid peer id %d peer stats buffer\n",
                    peer_id);
        goto out;
    }
...
bool ath10k_htt_t2h_msg_handler(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (!IS_ALIGNED((unsigned long)skb->data, 4)) {
        ath10k_info("unaligned htt message, expect trouble\n");
    }
    ...
    case HTT_T2H_MSG_TYPE_TX_INSPECT_IND:
         ...
         */
        ath10k_info("received an unexpected htt tx inspect event\n");
        break;
    ...
    case HTT_T2H_MSG_TYPE_TX_FETCH_IND: {
        ...
        if (!tx_fetch_ind) {
            ath10k_info("failed to copy htt tx fetch ind\n");
            break;
        }
    ...
    default:
        ath10k_info("htt event (%d) not handled\n",
                    resp->hdr.msg_type);
        ath10k_dbg_dump(ar, ATH10K_DBG_HTT_DUMP, NULL, "htt event: ",
                        skb->data, skb->len);
        break;
