...
static void ath10k_report_offchan_tx(struct ath10k* ar, struct sk_buff* skb) {
    ...
    if (ar->offchan_tx_skb != skb) {
        ath10k_info("completed old offchannel frame\n");
        goto out;
    }
...
int ath10k_txrx_tx_unref(struct ath10k_htt* htt,
    ...
    if (tx_done->msdu_id >= htt->max_num_pending_tx) {
        ath10k_info("warning: msdu_id %d too big, ignoring\n",
                    tx_done->msdu_id);
        return -EINVAL;
    }
    ...
    if (!msdu) {
        ath10k_info("received tx completion for invalid msdu_id: %d\n",
                    tx_done->msdu_id);
...
void ath10k_peer_map_event(struct ath10k_htt* htt,
    ...
    if (ev->peer_id >= ATH10K_MAX_NUM_PEER_IDS) {
        ath10k_info("received htt peer map event with idx out of bounds: %hu\n",
                    ev->peer_id);
        return;
    }
...
void ath10k_peer_unmap_event(struct ath10k_htt* htt,
    ...
    if (ev->peer_id >= ATH10K_MAX_NUM_PEER_IDS) {
        ath10k_info("received htt peer unmap event with idx out of bounds: %hu\n",
                    ev->peer_id);
        return;
    }
    ...
    if (!peer) {
        ath10k_info("peer-unmap-event: unknown peer id %d\n",
                    ev->peer_id);
        goto exit;
    }
