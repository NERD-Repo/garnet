...
static void __ath10k_htt_tx_txq_recalc(struct ieee80211_hw* hw,
    ...
    if (unlikely(peer_id >= ar->htt.tx_q_state.num_peers) ||
        ...
        ath10k_info("refusing to update txq for peer_id %hu tid %hhu due to out of bounds\n",
                    peer_id, tid);
        return;
    }
...
static int ath10k_htt_tx_alloc_txq(struct ath10k_htt* htt) {
    ...
    if (ret) {
        ath10k_info("failed to dma map tx_q_state: %d\n", ret);
        kfree(htt->tx_q_state.vaddr);
        return -EIO;
    }
...
int ath10k_htt_h2t_stats_req(struct ath10k_htt* htt, uint8_t mask, uint64_t cookie) {
    ...
    ret = ath10k_htc_send(&htt->ar->htc, htt->eid, skb);
    if (ret) {
        ath10k_info("failed to send htt type stats request: %d",
                    ret);
        dev_kfree_skb_any(skb);
        return ret;
    }
...
int ath10k_htt_send_frag_desc_bank_cfg(struct ath10k_htt* htt) {
    ...
    if (!htt->frag_desc.paddr) {
        ath10k_info("invalid frag desc memory\n");
        return -EINVAL;
    }
    ...
    ret = ath10k_htc_send(&htt->ar->htc, htt->eid, skb);
    if (ret) {
        ath10k_info("failed to send frag desc bank cfg request: %d\n",
                    ret);
        dev_kfree_skb_any(skb);
        return ret;
    }
...
int ath10k_htt_tx_fetch_resp(struct ath10k* ar,
    ...
    if (ret) {
        ath10k_info("failed to submit htc command: %d\n", ret);
        goto err_free_skb;
    }
