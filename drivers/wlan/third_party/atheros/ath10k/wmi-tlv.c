...
static int ath10k_wmi_tlv_event_bcn_tx_status(struct ath10k* ar,
    ...
    if (IS_ERR(tb)) {
        ret = PTR_ERR(tb);
        ath10k_info("failed to parse tlv: %d\n", ret);
        return ret;
    }
    ...
    case WMI_TLV_BCN_TX_STATUS_FILTERED:
         ...
         */
        ath10k_info("received bcn tmpl tx status on vdev %i: %d",
                    vdev_id, tx_status);
        break;
    }
...
static int ath10k_wmi_tlv_event_diag_data(struct ath10k* ar,
    ...
    while (num_items--) {
        ...
        if (len < sizeof(*item)) {
            ath10k_info("failed to parse diag data: can't fit item header\n");
            break;
        }
        ...
        if (len < sizeof(*item) + item->len) {
            ath10k_info("failed to parse diag data: item is too long\n");
            break;
        }
    ...
    if (num_items != -1 || len != 0)
        ath10k_info("failed to parse diag data event: num_items %d len %d\n",
                    num_items, len);
...
static int ath10k_wmi_tlv_op_pull_svc_rdy_ev(struct ath10k* ar,
    ...
    if (ret) {
        kfree(tb);
        ath10k_info("failed to parse mem_reqs tlv: %d\n", ret);
        return ret;
    }
