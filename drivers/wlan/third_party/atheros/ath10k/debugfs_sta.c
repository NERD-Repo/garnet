...
static ssize_t ath10k_dbg_sta_write_aggr_mode(struct file* file,
    ...
    if (ret) {
        ath10k_info("failed to clear addba session ret: %d\n", ret);
        goto out;
    }
...
static ssize_t ath10k_dbg_sta_write_addba(struct file* file,
    ...
    ret = ath10k_wmi_addba_send(ar, arsta->arvif->vdev_id, sta->addr,
                                tid, buf_size);
    if (ret) {
        ath10k_info("failed to send addba request: vdev_id %u peer %pM tid %u buf_size %u\n",
...
static ssize_t ath10k_dbg_sta_write_addba_resp(struct file* file,
    ...
    ret = ath10k_wmi_addba_set_resp(ar, arsta->arvif->vdev_id, sta->addr,
                                    tid, status);
    if (ret) {
        ath10k_info("failed to send addba response: vdev_id %u peer %pM tid %u status%u\n",
...
static ssize_t ath10k_dbg_sta_write_delba(struct file* file,
    ...
    if (ret) {
        ath10k_info("failed to send delba: vdev_id %u peer %pM tid %u initiator %u reason %u\n",
...
ath10k_dbg_sta_write_peer_debug_trigger(struct file* file,
    ...
    if (ret) {
        ath10k_info("failed to set param to trigger peer tid logs for station ret: %d\n",
                    ret);
        goto out;
    }
out:
