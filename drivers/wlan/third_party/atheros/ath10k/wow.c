...
static int ath10k_wow_vif_cleanup(struct ath10k_vif* arvif) {
    ...
    for (i = 0; i < WOW_EVENT_MAX; i++) {
        ...
        if (ret) {
            ath10k_info("failed to issue wow wakeup for event %s on vdev %i: %d\n",
    ...
    for (i = 0; i < ar->wow.max_num_patterns; i++) {
        ...
        if (ret) {
            ath10k_info("failed to delete wow pattern %d for vdev %i: %d\n",
                        i, arvif->vdev_id, ret);
            return ret;
        }
    }
...
static int ath10k_wow_cleanup(struct ath10k* ar) {
    ...
    list_for_each_entry(arvif, &ar->arvifs, list) {
        ...
        if (ret) {
            ath10k_info("failed to clean wow wakeups on vdev %i: %d\n",
                        arvif->vdev_id, ret);
            return ret;
        }
    }
...
static int ath10k_vif_wow_set_wakeups(struct ath10k_vif* arvif,
    ...
    for (i = 0; i < wowlan->n_patterns; i++) {
        ...
        if (ret) {
            ath10k_info("failed to add pattern %i to vdev %i: %d\n",
                        pattern_id,
                        arvif->vdev_id, ret);
            return ret;
        }
    ...
    for (i = 0; i < WOW_EVENT_MAX; i++) {
        ...
        if (ret) {
            ath10k_info("failed to enable wakeup event %s on vdev %i: %d\n",
                        wow_wakeup_event(i), arvif->vdev_id, ret);
            return ret;
        }
    }
...
static int ath10k_wow_set_wakeups(struct ath10k* ar,
    ...
    list_for_each_entry(arvif, &ar->arvifs, list) {
        ...
        if (ret) {
            ath10k_info("failed to set wow wakeups on vdev %i: %d\n",
                        arvif->vdev_id, ret);
            return ret;
        }
    }
...
static int ath10k_wow_enable(struct ath10k* ar) {
    ...
    if (ret) {
        ath10k_info("failed to issue wow enable: %d\n", ret);
        return ret;
    }

    ret = wait_for_completion_timeout(&ar->target_suspend, 3 * HZ);
    if (ret == 0) {
        ath10k_info("timed out while waiting for suspend completion\n");
        return -ETIMEDOUT;
    }
...
static int ath10k_wow_wakeup(struct ath10k* ar) {
    ...
    if (ret) {
        ath10k_info("failed to send wow wakeup indication: %d\n",
                    ret);
        return ret;
    }
    ...
    if (ret == 0) {
        ath10k_info("timed out while waiting for wow wakeup completion\n");
        return -ETIMEDOUT;
    }
...
int ath10k_wow_op_suspend(struct ieee80211_hw* hw,
    ...
    if (ret) {
        ath10k_info("failed to clear wow wakeup events: %d\n",
                    ret);
        goto exit;
    }
    ...
    if (ret) {
        ath10k_info("failed to set wow wakeup events: %d\n",
                    ret);
        goto cleanup;
    }
    ...
    if (ret) {
        ath10k_info("failed to start wow: %d\n", ret);
        goto cleanup;
    }

    ret = ath10k_hif_suspend(ar);
    if (ret) {
        ath10k_info("failed to suspend hif: %d\n", ret);
        goto wakeup;
    }
...
int ath10k_wow_op_resume(struct ieee80211_hw* hw) {
    ...
    if (ret) {
        ath10k_info("failed to resume hif: %d\n", ret);
        goto exit;
    }

    ret = ath10k_wow_wakeup(ar);
    if (ret) {
        ath10k_info("failed to wakeup from wow: %d\n", ret);
    }
...
exit:
    if (ret) {
        ...
        case ATH10K_STATE_WEDGED:
            ath10k_info("encountered unexpected device state %d on resume, cannot recover\n",
                        ar->state);
            ret = -EIO;
            break;
        }
