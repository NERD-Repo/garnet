...
void Device::StartScan(wlan_mlme::ScanRequest req) {
    ...
    impl_req.channel_list = req.channel_list->data();
    ...
    for (size_t ndx = 0; ndx < num_ssids; ndx++) {
        ssids[ndx] = (*req.ssid_list)[ndx]->c_str();
    }
    impl_req.num_ssids = num_ssids;
    impl_req.ssid_list = ssids;
...
void Device::JoinReq(wlan_mlme::JoinRequest req) {
    ...
    impl_req.op_rates = req.op_rate_set->data();
...
void Device::AssociateReq(wlan_mlme::AssociateRequest req) {
    ...
    if (is_protected) {
        impl_req.rsne_len = req.rsn->size();
        impl_req.rsne = req.rsn->data();
    } else {
        impl_req.rsne_len = 0;
        impl_req.rsne = nullptr;
    }

    {
        std::lock_guard<std::mutex> lock(lock_);
        data_state_ = is_protected ? ASSOC_REQ_RSN : ASSOC_REQ_OPEN;
    }
...
void Device::AssociateInd(wlan_mlme::AssociateIndication ind) {
    ...
    // ssid
    impl_ind.ssid_len = ind.ssid->size();
    impl_ind.ssid = ind.ssid->data();

    // rsne
    impl_ind.rsne_len = ind.rsn->size();
    impl_ind.rsne = ind.rsn->data();
...
void Device::StartReq(wlan_mlme::StartRequest req) {
    ...
    // rsne
    impl_req.rsne_len = req.rsne->size();
    impl_req.rsne = req.rsne->data();
...
void Device::SetKeysReq(wlan_mlme::SetKeysRequest req) {
    ...
    // keylist
    set_key_descriptor_t descriptors[num_keys];
    for (size_t desc_ndx = 0; desc_ndx < num_keys; desc_ndx++) {
        ConvertSetKeyDescriptor(&descriptors[desc_ndx], &(*req.keylist)[desc_ndx]);
    }
    impl_req.keylist = descriptors;
...
void Device::DeleteKeysReq(wlan_mlme::DeleteKeysRequest req) {
    ...
    // keylist
    delete_key_descriptor_t descriptors[num_keys];
    for (size_t desc_ndx = 0; desc_ndx < num_keys; desc_ndx++) {
        ConvertDeleteKeyDescriptor(&descriptors[desc_ndx], &(*req.keylist)[desc_ndx]);
    }
    impl_req.keylist = descriptors;
...
void Device::AuthenticateConf(wlanif_auth_confirm_t* resp) {
    std::lock_guard<std::mutex> lock(lock_);
    std::printf(" * * AuthConf\n");
    if (resp->result_code == AUTH_RESULT_SUCCESS && data_state_ != ONLINE) {
        EthStatus(ETH_STATUS_ONLINE);
        data_state_ = ONLINE;
    }
...
void Device::AssociateConf(wlanif_assoc_confirm_t* resp) {
    ...
    if (resp->result_code == ASSOC_RESULT_SUCCESS && data_state_ == ASSOC_REQ_OPEN) {
        EthStatus(ETH_STATUS_ONLINE);
        data_state_ = ONLINE;
    }
...
void Device::StartConf(wlanif_start_confirm_t* resp) {
    ...
    if (resp->result_code == START_RESULT_SUCCESS && data_state_ != ONLINE) {
        EthStatus(ETH_STATUS_ONLINE);
        data_state_ = ONLINE;
    }
...
void Device::EthStatus(uint32_t status) {
    if (eth_started_) {
        ethmac_ifc_.status(ethmac_cookie_, status);
    }
}

void Device::EthRecv(void* data, size_t length, uint32_t flags) {
    std::lock_guard<std::mutex> lock(lock_);
    std::printf(" * * EthRecv got data 0x%016lx, length %ld. Started %d\n", *(uint64_t*)data, length,
                eth_started_);
    if (eth_started_) {
