...
//    zxlogf(INFO, "%s header size %d\n", module_name, header_desc->bLength);
//
//    return header_desc->bcdCDC >= CDC_SUPPORTED_VERSION;
//    return header_desc->bcdCDC >= CDC_SUPPORTED_VERSION;
//}
//
//static bool parse_cdc_union_descriptor(ecm_ctx_t* ctx, usb_cs_union_interface_descriptor_1_t* desc) {
//    zxlogf(INFO, "%s union size %d\n", module_name, desc->bLength);
...
static zx_status_t qmi_bind(void* ctx, zx_device_t* device) {
    zxlogf(INFO, "BWB: Loading qmi_bind\n");
    ...
    if (!ecm_ctx) {
        zxlogf(ERROR, "qmi: failed to allocate memory for USB CDC driver\n");
        return ZX_ERR_NO_MEMORY;
    }

    result = usb_claim_additional_interfaces(&usb, want_interface, NULL);
    if (result != ZX_OK) {
        zxlogf(ERROR, "qmi: failed to claim CLASS_CDC interface\n");
        goto fail;
    }
    ...
    usb_cs_union_interface_descriptor_1_t* cdc_union_desc = NULL;
    ...
    while (desc) {
        if (desc->bDescriptorType == USB_DT_INTERFACE) {
          ...
          printf("Descriptor subtype: 0x%02hhx\n", cs_ifc_desc->bDescriptorSubType);
          if (cs_ifc_desc->bDescriptorSubType == 0x00) { /* HEADER */
            printf("header\n");
          ...
          if (cs_ifc_desc->bDescriptorSubType == 0x06) { /* Union Desc */
            printf("union\n");
    ...
    zxlogf(INFO, "%s: device reports CDC version as 0x%x\n", module_name, cdc_header_desc->bcdCDC);
